...
namespace EmuLibDebugger
{
    class FUNC_Additional_Drawing
    {
        ...
        private static Rect DebugWindow = new Rect(10f, 10f, 450f, 200f);
        private static Rect DragWindowDebugWindow = new Rect(0, 0, DebugWindow.width, 20);
        public static void DrawCordsHud(int id)
        {
            try
            {
                ...
                Drawing.Label("EmuTarkov Coords", 0, 0, Constants.Locations.boxSize.box_100, Constants.Locations.boxSize.box_20);
                // First column
                //column_3 = 2;
                Drawing.Label(" ---Aiming at--- ", column_2, 1);
                Drawing.Label("X:" + Camera.main.transform.position.x.ToString(), column_2++, 1);
                Drawing.Label("Y:" + Camera.main.transform.position.y.ToString(), column_2++, 1);
                Drawing.Label("Z:" + Camera.main.transform.position.z.ToString(), column_2++, 1);

               //Disabled for now
               Drawing.Label(" ---PlayerPosition--- ", column_2++, 1);
               Drawing.Label("X:" + Cons.Main._localPlayer.Transform.position.x.ToString(), column_1++, 1);
               Drawing.Label("Y:" + Cons.Main._localPlayer.Transform.position.y.ToString(), column_1++, 1);
               Drawing.Label("Z:" + Cons.Main._localPlayer.Transform.position.z.ToString(), column_1++, 1);*/
                GUILayout.Label(" - Aiming at - ");
                GUILayout.Label(" X: " + Camera.main.transform.position.x.ToString());
                GUILayout.Label(" Y: " + Camera.main.transform.position.y.ToString());
                GUILayout.Label(" Z: " + Camera.main.transform.position.z.ToString());
                GUILayout.Label(" - PlayerPosition - ");
                GUILayout.Label(" X: " + Cons.Main._localPlayer.Transform.position.ToString());
                GUILayout.Label(" Y: " + Cons.Main._localPlayer.Transform.position.ToString());
                GUILayout.Label(" Z: " + Cons.Main._localPlayer.Transform.position.ToString());
                GUI.color = guiBackup;
            }
            catch (Exception e)
            {
                ErrorHandler.Catch("DrawCordsHud", e);
            }
        }
            ...
            public static void DrawHUDMenu()
            {
                ...
                int column_1 = 0, column_2 = 0;//, column_3 = 0;
                    ...
                    GUI.color = Color.white;
                    Vector2 initial = new Vector2(15f, 20f);
                    Drawing.Label("EmuTarkov", 0, 0, Constants.Locations.boxSize.box_100, Constants.Locations.boxSize.box_20);
                    // First column
                    column_1 = 2;
                    column_2 = 2;
                    //column_3 = 2;
                    ...
                    Drawing.CheckBox(ref Cons.Bools.IKnowWhatImDoing, "Tele", 3, 2);
                    // start what maoci starts each time
                    Drawing.Button(ref Cons.Buttons.DebugBut, "Debug", 4, 2);
